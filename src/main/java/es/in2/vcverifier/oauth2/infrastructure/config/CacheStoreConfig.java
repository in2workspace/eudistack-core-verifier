...
public class CacheStoreConfig {
    ...
    // EUD-149: the SSO credential snapshot used to live here (CacheStore<JsonNode>, in-memory,
    // non-distributed) — removed. It now persists encrypted as sso_session.credential_snapshot
    // (see AesGcmSsoCredentialCipherAdapter / EstablishSsoSessionWorkflow), so a reuse landing on
    // a different replica than the one that established the session can still decrypt it.
