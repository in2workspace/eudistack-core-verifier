...
CREATE INDEX IF NOT EXISTS idx_sso_audit_event_tenant_occurred ON sso_audit_event (tenant, occurred_at);

-- Prevent updates/deletes to enforce write-once semantics
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'sso_audit_prevent_mods') THEN
        CREATE FUNCTION sso_audit_prevent_mods() RETURNS trigger LANGUAGE plpgsql AS $$
        BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'trg_sso_audit_prevent_mods'
    ) THEN
        CREATE TRIGGER trg_sso_audit_prevent_mods
            BEFORE UPDATE OR DELETE ON sso_audit_event
            FOR EACH ROW EXECUTE FUNCTION sso_audit_prevent_mods();
    END IF;
END$$;
