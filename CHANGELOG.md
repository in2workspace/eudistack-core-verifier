...
- **DCQL login queries only constrained the credential type, never the power required to use it**: a wallet could offer, and the holder could present, a `learcredential.employee` credential that lacked the power the relying party actually needed — the rejection only surfaced afterwards, at the relying party. `DcqlProfileResolverImpl` now forwards a profile's configured `claims` into the resolved query instead of discarding them (this was already silently broken for the existing `doctorid` profile, which declared `claims` in config with no effect). New `issuer_access` profile expresses the Issuer console's own requirement — an Onboarding/Execute power, or the SysAdmin power, on `mandate.power[]` — as two alternative credential entries per format, so a wallet can exclude non-qualifying credentials before presentation.
