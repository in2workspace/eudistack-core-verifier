...
- **Login QR countdown froze around 30s remaining in DEV/STG/PROD — CloudFront killed the idle SSE stream with a 504**: the login SSE stream (`/api/login/events`) sends no bytes at all while waiting for the wallet, and CloudFront's default `origin_read_timeout` (30s), retried up to 3x before giving up (~90s of total silence), was terminating the connection with a `504` well before the 120s login window elapsed. The client's `EventSource` couldn't recover cleanly from that failure mode, leaving the countdown UI stuck instead of showing a graceful timeout. `SseEmitterStore` now schedules a periodic SSE comment (`:keep-alive`, every 15s, via the app's existing `TaskScheduler`) for the lifetime of each emitter, cancelled on completion/timeout/error/send — verified live against a running instance (`curl -N .../api/login/events?state=...`) that `:keep-alive` bytes actually arrive on the wire every ~15s. Tests: `SseEmitterStoreTest` (heartbeat scheduled on `create`, sends a comment when invoked, cancels itself if the emitter is already gone, cancelled by `send`/`sendValidationFailed`/timeout).
- **`SseEmitterStore` timed-out login SSE stream surfaced as a raw 503 instead of a clean close**: `LoginSseController`'s emitter (`SseEmitterStore.create`) never called `emitter.complete()` in `onTimeout`, so on the legitimate 120s login timeout Spring's default async-timeout handling completed the request with a raw `503` — which the `eudistack-mfe-login` client's `EventSource` surfaced as a connection error, racing the client's own independent 120s countdown/redirect instead of a clean stream close. `onTimeout` now calls `emitter.complete()` after removing the emitter from the store. Test: `SseEmitterStoreTest.onTimeout_completesEmitterAndRemovesIt` (Mockito `mockConstruction`, asserts `complete()` is called and the emitter is removed before any subsequent `send()`).
- **EUD-149 — Removed tenant-specific `search_path` management in `SsoSessionJdbcRepository` (H-10)**: `SsoSessionJdbcRepository` previously executed `SET LOCAL search_path = "<tenant>", public` on each transactional operation under the assumption of a schema-per-tenant architecture. In DEV/STG/PROD environments, IaC deploys the Verifier against a single shared schema `verifier` (`SPRING_FLYWAY_DEFAULT_SCHEMA=verifier` and `currentSchema=verifier` on the datasource connection URL) with no per-tenant schemas created. Setting `search_path` to `"<tenant>", public` shadowed the connection default schema, causing all unqualified table queries (`sso_session`, `sso_session_client`) to fail with `PSQLException: relation "sso_session" does not exist` when `ssoEnabled=true`. Removed `setTenantSearchPath` and the `tenant` parameter in `inTransaction(op)` so queries execute against the shared schema; multi-tenant isolation continues to be enforced strictly via explicit `WHERE tenant = ?` SQL clauses in all repository queries (with `findById` intentionally querying without tenant filter for cross-tenant detection). Preserved transaction management, statement timeout (`statement_timeout`), and connection lifecycle handling. Tests: updated unit tests in `SsoSessionJdbcRepositoryTest`.
...
- **EUD-149 — producción-readiness: snapshot de credencial SSO cifrado y persistido, no cacheado en memoria**: sustituye `CacheStore<JsonNode> ssoSessionCredentialCache` (memoria local al proceso, no distribuida entre réplicas — una reutilización que aterrizaba en una réplica distinta a la que estableció la sesión encontraba siempre un cache miss y fallaba en silencio a `login_required`, aunque la fila en Postgres siguiera `ACTIVE` y válida) por una columna `sso_session.credential_snapshot` (`BYTEA`, migración `V8`), cifrada con AES-256-GCM (`AesGcmSsoCredentialCipherAdapter` / `SsoCredentialCipherPort`) usando `tenant`+`sessionId` como AAD. `EstablishSsoSessionWorkflow` cifra y adjunta el snapshot a `SsoSession` antes de `save()` (misma fila/transacción); `ReuseSsoSessionWorkflowImpl` lo descifra al reutilizar. La clave (`VERIFIER_SSO_CREDENTIAL_ENCRYPTION_KEY`, base64 de 32 bytes) debe ser la MISMA en toda réplica que comparta la BD de un tenant — vacía en dev genera una efímera por proceso (solo instancia única, documentado). Tests: `AesGcmSsoCredentialCipherAdapterTest` (10 casos: round-trip, IV no determinista, clave/tenant/sessionId incorrectos, ciphertext manipulado, clave ausente/inválida), `SsoCredentialSnapshotMultiInstanceIT` (establece en una instancia, reutiliza en otra completamente separada — sin ningún objeto ni caché compartidos — verificando `ALLOWED` con clave compartida y fail-closed a `LOGIN_REQUIRED` con clave distinta).

- **EUD-149 — FR-21/AC-09: autenticación fresca forzada por `max_age`**: `CustomAuthorizationRequestConverter` ahora parsea el parámetro OIDC `max_age` (segundos, entero no negativo; ausente o inválido se ignora) y lo propaga vía el nuevo campo `AuthorizationContext.maxAge`. En la rama de reutilización silenciosa SSO (`prompt=none`), `ReuseSsoSessionWorkflowImpl` pasa ese valor a `TenantSsoPolicy.evaluate(...)`, que devuelve el nuevo `ReuseDecision.REJECT_MAX_AGE` cuando la sesión sigue `ACTIVE` y dentro del TTL absoluto pero es más antigua que lo que el cliente exige explícitamente — se mapea a `login_required` igual que `REJECT_SESSION`, pero queda auditable por separado (`outcome=REJECT_MAX_AGE`). `prompt=login` seguía cubierto implícitamente (salta la rama de reutilización por completo) y no cambia. Tests: `TenantSsoPolicyTest` (4 casos nuevos), `ReuseSsoSessionIT` (2 casos HTTP end-to-end).
...
- **Uncontrolled OIDC logout exposed a raw JSON error to the browser**: when the standard `id_token_hint`/`post_logout_redirect_uri` validation of `GET/POST /oidc/logout` rejected a request (e.g. an expired `id_token_hint` after a session timeout), the response was the raw `OAuth2Error` JSON body instead of a guided screen. `SsoSessionLogoutFailureHandler` now resolves the requesting client via the `client_id` parameter and, when it has a `loginPageUri` configured, redirects there with `?error=session_expired` instead of exposing the error; clients without a configured login page keep the previous behavior unchanged. Tests: `SsoSessionLogoutFailureHandlerTest` (new, unit) and a new case in `SingleLogoutValidationIT`.
- **EUD-149 — `perf/verifier-sso.js` never actually measured SSO establishment overhead (NFR-P-149-02)**: `measureEstablishOverhead()` and `default()` passed `state`/`vp_token` as the 3rd argument (`params`) of `http.post`, which in k6's API is for headers/tags/redirects — never the request body. Those form fields were silently never sent, so the server rejected every request with a `500` (`MissingServletRequestParameterException`) before reaching any SSO/OID4VP logic; the recorded p95 measured the cost of rejecting an invalid request, not of establishing a session. Fixed by building a real `application/x-www-form-urlencoded` body (2nd argument) in both functions.
...
- **EUD-149 — code review follow-ups (W1/W2/W3/B2)**:
  - **[W1] Ephemeral AES key opaquely broke multi-replica reuse**: `AesGcmSsoCredentialCipherAdapter` now fails fast at startup (`IllegalStateException`, refuses to start) when `VERIFIER_SSO_CREDENTIAL_ENCRYPTION_KEY` is unset **and** at least one tenant has `ssoEnabled=true` — new `TenantSsoConfigPort.hasAnyTenantSsoEnabled()`. Still falls back to a per-process ephemeral key (with the existing loud warning) when no tenant has SSO active, so dev/test environments that never exercise the flag keep working. Local `eudistack-platform-dev` compose and `application-test.yaml` now set a fixed key, since the classpath `sso-config.yaml`/local tenants have SSO enabled and would otherwise fail to start.
  - **[W2] Fail-open on credential-snapshot encryption failure (ES-02)**: `EstablishSsoSessionWorkflow.attachCredentialSnapshot` no longer swallows a missing `credentialJson` or an `encrypt()` failure — both now throw and are caught by the same fail-closed path already used for persistence failures (`SSO_PERSIST_ERROR`, no session, no cookie), instead of silently persisting a session + cookie backed by no usable snapshot.
  - **[W3] `max_age` with no ceiling could 500 instead of `login_required`**: `Long.MAX_VALUE` (or anything above the 24h max absolute TTL) overflowed `Instant.plusSeconds` in `TenantSsoPolicy`. `CustomAuthorizationRequestConverter.parseMaxAge` now rejects (logs + ignores, same as a negative value) anything above `SsoTtlRange.MAX_ABSOLUTE`; `TenantSsoPolicy.evaluate` also catches the overflow defensively and resolves to `REJECT_MAX_AGE` for any future caller that skips that cap.
  - **[B2] Partial `sub` leak in audit events (AC-10/NFR-S-149-01)**: `SsoSessionAuthenticationSuccessHandler` published its own `SSO_SESSION_ESTABLISHED`/`SSO_ESTABLISH_FAILED` events using the raw `sub` (`SsoAuditAdapter.prefix()` truncates `holderHash` for the `holderHashPrefix` log field without re-hashing it). The handler now hashes it once via `HashingService` before every audit publish, and no longer publishes a duplicate `SSO_SESSION_ESTABLISHED` on success — `EstablishSsoSessionWorkflow` already publishes the correctly-hashed one in the same transaction as the persisted row.
  - **[B1] Verified, not a defect**: re-reviewed the claim that `Oid4vpController.buildSsoAuthentication` never populates `credentialJson` — `handleAuthResponse` returns `JsonNode` (not `void`) and is already captured and forwarded (present since `24108c6`, pre-dating this Story). Added `EstablishThenReuseRealLoginPathIT` (new), the first IT to exercise the real `POST /oid4vp/auth-response` entry point end-to-end (no JDBC-seeded snapshot): asserts `credential_snapshot IS NOT NULL` after a real establishment, then a `prompt=none` reuse issues a real `code=`.
  - Tests: `EstablishSsoSessionWorkflow_CredentialSnapshotTest`, `SsoSessionAuthenticationSuccessHandlerTest`, `TenantSsoPolicyTest`, `CustomAuthorizationRequestConverterTest`, `AesGcmSsoCredentialCipherAdapterTest` (updated for the new fail-fast constructor dependency), `EstablishThenReuseRealLoginPathIT` (new).
