...
| --- | --- |-------------------------------------------------------------------------------------------------------------------------------------|
| `VERIFIER_BACKEND_URL` | `http://localhost:8082` | URL publica canonica del Verifier. Se usa como `iss`/audiencia en los tokens emitidos y como base para endpoints OID4VP. **Obligatorio en produccion.** |
| `VERIFIER_BACKEND_ADDITIONALURLS` | *(vacio)* | Dominios alias adicionales del Verifier, separados por coma (ej. `https://a.com,https://b.com`), cuando es accesible bajo varios hostnames (SSO multi-tenant/multi-app). No afectan a `iss`/audiencia (siempre viene de `VERIFIER_BACKEND_URL`); junto con este, son origenes confiables para las redirecciones propias del Verifier (`/login`, `/error`). |
